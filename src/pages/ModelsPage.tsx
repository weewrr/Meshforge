...
function toExt(e: WorkflowExtension): Ext {
  ...
  return {
    ...
    description: isModel
      ? 'Generator registered on the Python server.'
      : 'Mesh processing tool registered on the Python server.',
    author: 'meshforge',
...
function ExtensionCard({ ext, onOpen, onUninstall }: { ext: Ext; onOpen: (ext: Ext) => void; onUninstall: (ext: Ext) => void }) {
  const isModel = ext.type === 'model'

  const status = ext.loaded
    ? { tone: 'green' as const, text: isModel ? 'All nodes ready' : 'Ready' }
    : { tone: 'amber' as const, text: 'Not loaded' }
...
function ExtensionDrawer({ ext, onClose, onUninstall }: { ext: Ext; onClose: () => void; onUninstall: (ext: Ext) => void }) {
  const isModel = ext.type === 'model'
    ...
    <>
      ...
      <aside ref={trapRef} role="dialog" aria-modal="true" aria-label={ext.name} className="ex-drawer">
        <div className="ex-drawer__head">
          <button onClick={onClose} title="Close" aria-label="Close" className="ex-drawer__close">
          ...
          <div className="ex-drawer__titlewrap">
            ...
            <div>
              ...
              <div className="ex-drawer__metarow">
                <TypePill type={ext.type} />
                {ext.version && <span className="ex-card__version">v{ext.version}</span>}
                {ext.author && <span className="ex-drawer__author">by {ext.author}</span>}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="ex-drawer__body">
          ...
          <div className="ex-drawer__field">
            <p className="ex-drawer__fieldlabel">Identifier</p>
            <code className="ex-drawer__code">{ext.id}</code>
          </div>

          <div className="ex-drawer__field">
            <p className="ex-drawer__fieldlabel">Status</p>
            <StatusBadge tone={ext.loaded ? 'green' : 'amber'}>
              {ext.loaded ? 'Loaded on the server' : 'Registered, not loaded'}
            </StatusBadge>
          </div>

          <div className="ex-drawer__field">
            <p className="ex-drawer__fieldlabel">Nodes</p>
    ...
    <div className="ex-install">
      <div className="ex-install__row">
        <span className="ex-install__label">
          {step === 'downloading' && `Downloading… ${percent ?? 0}%`}
          {step === 'extracting' && 'Extracting…'}
          {step === 'validating' && 'Validating…'}
          {step === 'setting_up' && (message || 'Setting up environment…')}
          {step === 'done' && 'Installed!'}
          {step === 'error' && 'Installation failed'}
        </span>
        {step === 'setting_up' && <span className="ex-install__hint">May take a few minutes</span>}
      </div>
    ...
    <div className="ex-modal" onMouseDown={(e) => { if (e.target === e.currentTarget && !busy) onCancel() }}>
      ...
      <div className="ex-modal__card">
        <div className="ex-modal__head">
          ...
          <div>
            <h2 className="ex-modal__title">Uninstall &ldquo;{ext.name}&rdquo;?</h2>
            <p className="ex-modal__sub">The extension folder will be permanently deleted from the server.</p>
          </div>
        </div>
        ...
        <div className="ex-modal__actions">
          <button className="ex-modal__cancel" onClick={onCancel} disabled={busy}>Cancel</button>
          <button className="ex-modal__confirm" onClick={() => onConfirm(ext)} disabled={busy}>
            {busy ? <span className="ex-spinner" /> : 'Uninstall'}
          </button>
        </div>
      </div>
    </div>
...
export default function ModelsPage() {
  ...
  useEffect(() => {
    ...
    installPollRef.current = setInterval(async () => {
      ...
      if (progress && (progress.step === 'done' || progress.step === 'error')) {
        ...
        if (progress.step === 'error') setGhErr(progress.message ?? 'Installation failed')
  ...
  async function handleGHInstall(): Promise<void> {
    ...
    try {
      ...
      log('info', `install extension: ${result.message}`)
  ...
  // ── Local folder install (webkitdirectory) ──────────────────────────────
  ...
  async function handleLocalInstall(): Promise<void> {
    ...
    input.onchange = async () => {
      ...
      // The root dir is the top-level folder selected (first webkitRelativePath segment).
      const root = files[0]?.webkitRelativePath?.split('/')[0] ?? 'extension'
      setGhErr(null)
      try {
        const result = await installExtensionLocal(files, root)
        if (!result.ok) { setGhErr(result.message); return }
        log('info', `install local extension: ${result.message}`)
        setGhOk(true)
  ...
  async function handleReload(): Promise<void> {
    ...
    if (!result.ok) log('warn', `reload extensions: ${result.message}`)
    await refresh()
  }
  ...
  async function handleUninstallConfirm(ext: Ext): Promise<void> {
    ...
    try {
      ...
      log('info', `uninstall extension: ${result.message}`)
  ...
  return (
    <div className="ex">
      ...
      <div className="ex-head">
        <div>
          <h1 className="ex-head__title">Extensions</h1>
          <p className="ex-head__subtitle">
            <b>{counts.all}</b> installed · <b>{counts.process}</b> processors, <b>{counts.model}</b> models
          </p>
        </div>
      ...
      <div className="ex-toolbar">
        <label className="ex-search">
          ...
          />
          {search ? (
            <button onClick={() => setSearch('')} className="ex-search__clear">
        ...
        <div className="ex-sort" ref={sortRef}>
          <button onClick={() => setSortOpen((o) => !o)} className="ex-sort__btn">
        ...
        <div className="ex-ghform">
          <div className="ex-ghform__box">
            ...
            <p className="ex-ghform__hint">
              The repo must contain a <span className="mono">manifest.json</span> and a <span className="mono">generator.py</span> at its root.
            </p>
          </div>
        </div>
      )}
        ...
        {extensions.length === 0 && !loading ? (
          <div className="ex-empty">
            ...
            <div className="ex-empty__text">
              <p className="ex-empty__title">No extensions installed</p>
              <p className="ex-empty__sub">Install from GitHub or drop into the Meshforge extensions directory.</p>
            </div>
          </div>
        ) : loading ? (
          ...
          <div className="ex-noresult">
            ...
            <p>No extension matches “{search}”</p>
          </div>
        ) : grouped ? (
          <>
            {processList.length > 0 && (
              <section className="ex-group">
                <div className="ex-group__head">
                  ...
                  <h2 className="ex-group__title">Processors</h2>
                  <span className="ex-group__count">{processList.length}</span>
                  <span className="ex-group__line" />
                </div>
                <div className="ex-grid">
                  {processList.map((ext) => (
                    <ExtensionCard key={ext.id} ext={ext} onOpen={(e) => setSelectedId(e.id)} onUninstall={openUninstallModal} />
                  ))}
                </div>
              </section>
            )}
            {modelList.length > 0 && (
              <section className={`ex-group ${processList.length > 0 ? 'ex-group--spaced' : ''}`}>
                <div className="ex-group__head">
                  ...
                  <h2 className="ex-group__title">Models</h2>
      ...
      {/* Detail drawer */}
      {selectedExt && (
        <ExtensionDrawer ext={selectedExt} onClose={() => setSelectedId(null)} onUninstall={openUninstallModal} />
      )}
